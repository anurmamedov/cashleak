...
final class WeekComparisonTests: XCTestCase {
    ...
    func testReturnsNilWithoutEnoughInEachWeek() {
        ...
        XCTAssertNil(AnalysisAggregates.weekOverWeek(sparse))
    }

    func testReturnsNilWhenLastWeekIsEmpty() {
        let onlyThisWeek = (0..<8).map { _ in transaction(20, daysAgo: 0, verdict: .leak) }
        XCTAssertNil(AnalysisAggregates.weekOverWeek(onlyThisWeek))
    }
    ...
    func testUnconfirmedAreExcluded() {
        ...
        guard let comparison = AnalysisAggregates.weekOverWeek(data) else {
    ...
    func testWeekdayOccurrencesCountsRealDays() {
        let counts = AnalysisAggregates.weekdayOccurrences(in: .month)
        XCTAssertEqual(counts.values.reduce(0, +), Calendar.current.component(.day, from: .now))
    ...
    func testNoWeekdayFindingFromATwoWeekTrial() {
        ...
        let finding = AnalysisAggregates.finding(data, range: .month)
        if let finding {
            XCTAssertFalse(
